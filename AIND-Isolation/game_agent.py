...
def custom_score(game, player):
    ...
    own_move_ability = sum([len(game.forecast_move(move).get_legal_moves(player)) for move in own_moves])
    opp_move_ability = sum([len(game.forecast_move(move).get_legal_moves(game.get_opponent(player))) for move in own_moves])

    return float(own_move_ability - opp_move_ability)
...
def custom_score_3(game, player):
    ...
    own_moves = len(game.get_legal_moves(player))
    opp_moves = len(game.get_legal_moves(game.get_opponent(player)))
    com_moves = own_moves and opp_moves
    return float(own_moves - opp_moves) - alpha * com_moves
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        return self.ab(game, depth, alpha=alpha, beta=beta)[0]
    ...
    def ab(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        for move in game.get_legal_moves():
            next_ply = game.forecast_move(move)
            if depth == 1:
                score = self.score(next_ply, self)
            else:
